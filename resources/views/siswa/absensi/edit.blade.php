...
@section('content')
    <div class="row">
        <div class="col-lg">
            <form action="{{ route('siswa-absensi.update', $absensis->id) }}" method="POST">
                @csrf
                <div class="card">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="eskul_id" class="form-control @error('eskul_id') is-invalid @enderror"
                                        ...
                                        @foreach ($eskuls as $data)
                                            <option value="{{ $data->eskul->id }}"
                                                {{ $absensis->eskul_id == $data->eskul->id ? 'selected' : '' }}>
                                                {{ $data->eskul->nama ?? '-' }}</option>
                                        @endforeach
                                    </select>
                                    @error('eskul_id')
