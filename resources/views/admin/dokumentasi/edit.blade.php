...
@section('content')
    <div class="row">
        <div class="col-lg">
            <form action="{{ route('data-dokumentasi.update', $dokumentasis->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="card">
                                    ...
                                    <select name="eskul_id" class="form-control @error('eskul_id') is-invalid @enderror"
                                        ...
                                        @foreach ($eskuls as $data)
                                            <option value="{{ $data->id }}"
                                                {{ $dokumentasis->eskul_id == $data->id ? 'selected' : '' }}>
                                                {{ $data->nama ?? '-' }}</option>
                                        @endforeach
                                    </select>
                                    @error('eskul_id')
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="pembina_id" class="form-control @error('pembina_id') is-invalid @enderror"
                                        ...
                                        @foreach ($pembinas as $data)
                                            <option value="{{ $data->id }}"
                                                {{ $dokumentasis->pembina_id == $data->id ? 'selected' : '' }}>
                                                {{ $data->nama ?? '-' }}</option>
                                        @endforeach
                                    </select>
                                    @error('pembina_id')
                            ...
                            <div class="col-lg">
                                <div class="mb-3">
                                    ...
                                    <input type="text" name="kegiatan"
                                        ...
                                        value="{{ old('kegiatan', $dokumentasis->kegiatan ?? '-') }}" placeholder="Masukan Nama Kelas">
                                    @error('kegiatan')
                                        <div class="invalid-feedback">
                                            {{ $message }}
                                        </div>
